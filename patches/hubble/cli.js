...
import { APP_VERSION, FARCASTER_VERSION, Hub, HubShutdownReason, S3_REGION, SNAPSHOT_S3_UPLOAD_BUCKET, } from "./hubble.js";
...
app
  ...
  .option("--log-individual-messages", "Log individual submitMessage. If disabled, log one line per second (default: disabled)")
  // Ethereum Options
  ...
  .option("--l2-first-block <number>", "The block number to begin syncing events from L2 Farcaster contracts", parseNumber)
  .option("--l2-chunk-size <number>", "The number of events to fetch from L2 Farcaster contracts at a time", parseNumber)
  .option("--l2-chain-id <number>", "The chain ID of the L2 Farcaster contracts are deployed to", parseNumber)
  .option("--l2-rent-expiry-override <number>", "The storage rent expiry in seconds to use instead of the default 1 year (ONLY FOR TESTS)", parseNumber)
  // Networking Options
  ...
  .option("--announce-server-name <name>", 'Server name announced to peers, useful if SSL/TLS enabled. (default: "none")')
  .option("--direct-peers <peer-multiaddrs...>", "A list of peers for libp2p to directly peer with (default: [])")
  .option("--rpc-rate-limit <number>", "RPC rate limit for peers specified in rpm. Set to -1 for none. (default: 20k/min)")
  ...
  .option("--catchup-sync-snapshot-message-limit <number>", `Difference in message count before triggering snapshot sync. (default: ${DEFAULT_CATCHUP_SYNC_SNAPSHOT_MESSAGE_LIMIT})`)
  // Metrics
  .option("--statsd-metrics-server <host>", 'The host to send statsd metrics to, eg "127.0.0.1:8125". (default: disabled)')
  // Opt-out Diagnostics Reporting
  .option("--opt-out-diagnostics [boolean]", "Opt-out of sending diagnostics data to the Farcaster foundation. " +
    "Diagnostics are used to troubleshoot user issues and improve health of the network. (default: disabled)")
  .option("--diagnostic-report-url <url>", `The URL to send diagnostic reports to. (default: ${DEFAULT_DIAGNOSTIC_REPORT_URL})`)
  // Debugging options
  .option("--disable-console-status", "Immediately log to STDOUT, and disable console status and progressbars. (default: disabled)")
  ...
  .option("--chunk-size <number>", `The number of blocks to batch when syncing historical events from Farcaster contracts. (default: ${DEFAULT_CHUNK_SIZE})`, parseNumber)
  ...
  .action(async (cliOptions) => {
    ...
    console.log("------------------------");
    startupCheck.printStartupCheckStatus(StartupCheckStatus.OK, `Farcaster: ${FARCASTER_VERSION} Hubble: ${APP_VERSION}`);
    ...
    const checkForProcessNumChange = () => {
      ...
      fs.readFile(`${processFileDir}${processFileName}`, "utf8", async (err, data) => {
        ...
        if (!isNaN(readProcessNum) && readProcessNum !== processNum) {
          logger.error({ readProcessNum, processNum }, `Another hub process started up with processNum ${readProcessNum}, exiting with SIGTERM`);
    ...
    if (cliOptions.id) {
      ...
      if (peerIdR.isErr()) {
        startupCheck.printStartupCheckStatus(StartupCheckStatus.ERROR, `Failed to read identity from ${cliOptions.id}. Please run "yarn identity create".`, "https://www.thehubble.xyz/intro/install.html#installing-hubble\n");
        process.exit(1);
      }
      else {
      ...
      if (peerIdR.isErr()) {
        startupCheck.printStartupCheckStatus(StartupCheckStatus.ERROR, `Failed to read identity from ${idFile}. Please run "yarn identity create".`, "https://www.thehubble.xyz/intro/install.html#installing-hubble\n");
        process.exit(1);
      }
      else {
        peerId = peerIdR.value;
    ...
    if (statsDServer) {
      ...
      startupCheck.printStartupCheckStatus(StartupCheckStatus.WARNING, "Hubble Monitoring is disabled", "https://www.thehubble.xyz/intro/install.html#monitoring-hubble");
      logger.info({}, "No statsd server specified. Statsd disabled");
    }
    const network = cliOptions.network ?? hubConfig.network;
    startupCheck.printStartupCheckStatus(StartupCheckStatus.OK, `Network is ${FarcasterNetwork[network]?.toString()}(${network})`);
    let testUsers;
    ...
    const hubAddressInfo = addressInfoFromParts(cliOptions.ip ?? hubConfig.ip, cliOptions.gossipPort ?? hubConfig.gossipPort ?? DEFAULT_GOSSIP_PORT);
    ...
    let bootstrapList = (cliOptions.bootstrap ?? hubConfig.bootstrap ?? []);
    ...
    const bootstrapAddrs = bootstrapList
      ...
      .map((a) => {
        if (a.isErr()) {
          logger.warn({ errorCode: a.error.errCode, message: a.error.message }, "Couldn't parse bootstrap address, ignoring");
        }
        return a;
      })
      .filter((a) => a.isOk())
    ...
    if (bootstrapAddrs.length > 0) {
      ...
      startupCheck.printStartupCheckStatus(StartupCheckStatus.WARNING, "No bootstrap peers specified. Hubble will not be able to sync without them.", "https://www.thehubble.xyz/intro/networks.html");
    }
    const directPeers = (cliOptions.directPeers ?? hubConfig.directPeers ?? [])
      .map((a) => parseAddress(a))
      .map((a) => {
        if (a.isErr()) {
          logger.warn({ errorCode: a.error.errCode, message: a.error.message }, "Couldn't parse direct peer address, ignoring");
        }
        else if (a.value.getPeerId()) {
          logger.warn({ errorCode: "unavailable", message: "peer id missing from direct peer" }, "Direct peer missing peer id, ignoring");
        }
        return a;
      })
    ...
    const options = {
      ...
      catchupSyncSnapshotMessageLimit: cliOptions.catchupSyncSnapshotMessageLimit ?? hubConfig.catchupSyncSnapshotMessageLimit,
      ...
      try {
        ...
        if (transfers && transfers.length > 0) {
          ...
          if (usernameField !== null && usernameField !== undefined) {
            ...
            startupCheck.printStartupCheckStatus(StartupCheckStatus.WARNING, `Hub Operator FID is ${fid}, but no username was found`);
          }
        }
      }
      catch (e) {
        logger.error(e, `Error fetching username for Hub Operator FID ${options.hubOperatorFid}`);
        startupCheck.printStartupCheckStatus(StartupCheckStatus.WARNING, `Hub Operator FID is ${options.hubOperatorFid}, but no username was found`);
      }
    }
    else {
      startupCheck.printStartupCheckStatus(StartupCheckStatus.WARNING, "Hub Operator FID is not set", "https://www.thehubble.xyz/intro/install.html#troubleshooting");
    }
    ...
    const hubResult = Result.fromThrowable(() => new Hub(options), (e) => new Error(`Failed to create hub: ${e}`))();
    if (hubResult.isErr()) {
      if (!startupCheck.anyFailedChecks()) {
    ...
    const startResult = await ResultAsync.fromPromise(hub.start(), (e) => new Error("Failed to start hub", { cause: e }));
...
const writePeerId = async (peerId, filepath) => {
...
app
  ...
  .option("-s, --server <url>", "Farcaster RPC server address:port to connect to (eg. 127.0.0.1:2283)", DEFAULT_RPC_CONSOLE)
...
const storageProfileCommand = new Command("storage")
  ...
  .action(async (cliOptions) => {
    ...
    if (!rocksDBName)
      throw new Error("No RocksDB name provided.");
    const dbResult = await ResultAsync.fromPromise(rocksDB.open(), (e) => e);
    if (dbResult.isErr()) {
...
app
  ...
  .action(async (cliOptions) => {
    ...
    if (dbResult.isErr()) {
      ...
      const clearResult = Result.fromThrowable(() => rocksDB.clear(), (e) => e)();
...
const readPeerId = async (filePath) => {
